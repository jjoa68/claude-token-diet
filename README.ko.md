...
10번째 메시지는 1번째의 11배 비용이 듭니다. Claude Code가 매 메시지마다 전체 대화, MCP 도구 정의, 규칙 파일을 처음부터 다시 읽기 때문이에요. 비용은 복리로 불어나고, 알아챘을 때는 이미 세션이 끝나 있죠.

저도 일주일치 토큰을 이틀 만에 다 태우고 나서야 원인을 찾았습니다.

`/token-diet`은 정확히 뭐가 토큰을 낭비하는지 찾아서 하나씩 고쳐나가는 과정을 안내해 드립니다. **10분이면 완성입니다. 코드나 하네스 엔지니어링을 몰라도 충분히 할 수 있어요.**
...
>
> 같은 작업, 같은 워크플로우. 차이는 `/token-diet` 10분뿐이에요.

## 뭘 하는 스킬인가요?

| Step | 소요 시간 | 내용 | 내가 얻는 것 |
|------|-----------|------|-------------|
| 1 | 1분 | `/clear`, `/compact` 습관 만들기 | 세션 중간에 토큰이 새는 걸 멈춤 |
| 2 | 3분 | 안 쓰는 MCP 정리, `.claudeignore` 생성 | 메시지당 컨텍스트 30~50% 감소 |
| 3 | 3분 | `rules/` 분리, Extended Thinking 조절 | 한도에 걸리기 전까지 더 긴 세션 |
| 4 | 3분 | 분산 메모리, ReadOnce 훅 | 한 주 내내 버티는 세션 |
